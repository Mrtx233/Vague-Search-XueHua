...
# 手动指定要处理的 JSON 文件绝对路径；为空则仍可通过命令行参数传入
JSON_INPUT_PATH = r""
...
class SnowflakeIdGenerator:
    ...
    def __init__(self, worker_id: int = 1):
        ...
        # 工作机器ID
        self.worker_id = worker_id & 0x3F  # 最大63
    ...
    def generate(self) -> str:
        ...
        if timestamp == self.last_timestamp:
            self.sequence = (self.sequence + 1) & 0xFFF  # 最大4095
            if self.sequence == 0:
        ...
        if len(snowflake_str) > 11:
            snowflake_str = snowflake_str[-11:]  # 取后11位
        elif len(snowflake_str) < 11:
            snowflake_str = snowflake_str.zfill(11)  # 补齐到11位
...
def resolve_json_input_file() -> str:
    manual_path = JSON_INPUT_PATH.strip()
    if manual_path:
        return os.path.abspath(manual_path)
    if len(sys.argv) > 1 and sys.argv[1].strip():
        return os.path.abspath(sys.argv[1].strip())
    raise SystemExit("请先在代码中填写 JSON_INPUT_PATH，或通过命令行传入 json 文件绝对路径")
...
class DrissionPageCrawlerManager:
    def __init__(self, base_dir: str, max_workers: int = 5, fasttext_model_path: Optional[str] = None,
        ...
        self.download_dir = self.base_dir / "output"
            ...
            if download_file(real_url, str(temp_save_path)):
                ...
                if md5_hash:
                    ...
                    try:
                        ...
                        # 创建路径: output/{雪花ID}/master/
                        snowflake_dir = download_dir / snowflake_id
                        master_dir = snowflake_dir / "master"
                        master_dir.mkdir(parents=True, exist_ok=True)

                        # 创建路径: output/{雪花ID}/meta/
                        meta_dir = snowflake_dir / "meta"
                        meta_dir.mkdir(parents=True, exist_ok=True)

                        # 生成文件名: md5.扩展名
                        final_filename = generate_filename_from_md5(md5_hash, result['extend']['type'])
                        final_save_path = master_dir / final_filename
                        ...
                        meta_save_path = meta_dir / meta_filename
                        with open(meta_save_path, 'w', encoding='utf-8') as f:
                            json.dump(result, f, ensure_ascii=False, indent=2)

                        logging.info(f"文件处理完成: {snowflake_id}/master/{final_filename} (MD5: {md5_hash})")
                        logging.info(f"元数据已保存: {snowflake_id}/meta/{meta_filename}")
                    except Exception:
...
if __name__ == '__main__':
    ...
    json_input_file = r'E:\Crawler\模糊搜索\模糊搜索\json\output\泰语\IT_A.json'
    file_type = 'xlsx'
    time_filter = ''
